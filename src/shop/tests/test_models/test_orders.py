...
@pytest.mark.django_db
def test_approve_order_insufficient_stock_after_stock_change(
    order_factory, order_item_factory, product_factory
):
    """
    ...
    - another admin approves a different order that reduces stock for the same product(orders
    are placed but not approved immediately)

...
from shop.models import Category, Order, Product, User
...
def user_customer(db):
    return User.objects.create_user(
        email="customer@example.com", password="testpassword123", role=User.CUSTOMER
    )
...
def product1(category):
    return Product.objects.create(
        name="Laptop", category=category, price=Decimal("1000.00"), stock=10, discount_price=Decimal("900.00")
    )
...
def product2(category):
    return Product.objects.create(
        name="Phone", category=category, price=Decimal("500.00"), stock=5, discount_price=Decimal("450.00")
    )
...
def test_order_serializer_valid(user_customer, product1, product2):
    ...
    data = {
        ...
        "order_items": [{"product": product1.id, "quantity": 2}, {"product": product2.id, "quantity": 1}],
    }
...
def test_order_status_defaults_to_pending(user_customer, product1, product2):
    ...
    data = {
        "customer": user_customer.id,
        "order_items": [{"product": product1.id, "quantity": 1}, {"product": product2.id, "quantity": 2}],
    }
    serializer = OrderSerializer(data=data)
    assert serializer.is_valid(), serializer.errors
    order = serializer.save()
...
def test_order_serializer_invalid_stock(user_customer, product1):
    ...
    """
    data = {"customer": user_customer.id, "order_items": [{"product": product1.id, "quantity": 20}]}
    serializer = OrderSerializer(data=data)
...
def test_order_readonly_fields(user_customer):
    ...
    data = {"customer": user_customer.id, "total_price": Decimal("1000.00")}  # Attempt to change total_price

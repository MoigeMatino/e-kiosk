...
class CustomUserAdmin(UserAdmin):  
    ...
    add_fieldsets = (  
        (None, {  
            'classes': ('wide',),  
            'fields': ('email', 'phone_number', 'role', 'openid_sub', 'is_active', 'is_staff', 'password1', 'password2'),  
        }),  
    )  
    ...
    def save_model(self, request, obj, form, change):  
        if not change:  # Only for new users  
            obj.set_unusable_password()  # so that no password is required for the user to log in(because customers are using oidc)
